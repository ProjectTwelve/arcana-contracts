...
contract TestRewardDistributor is IRewardDistributor, SafeOwnable, Initializable {
  ...
  /**
   * @dev used only in test
   */
  function setUnClaimed(address user) external {
    claimed.unset(uint160(user));
  }
