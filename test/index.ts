...
describe('P12Arcana', function () {
  ...
  it('Should update power successfully', async () => {
    ...
    const sig = await signer._signTypedData(domain, type, {
      tokenId: BigNumber.from(user.address),
      power: 100,
      deadline: deadline,
    });
    ...
    await p12Arcana
      .connect(user)
      ['updatePower(uint256,uint256,uint256,bytes)'](BigNumber.from(user.address), 100, deadline, sig);
  ...
  it('Should update power via meta transaction successfully', async () => {
    ...
    const tx = await p12Arcana
      .connect(user)
      .populateTransaction['updatePower(uint256,uint256,uint256,bytes)'](BigNumber.from(user.address), 200, deadline, sig);

    const req = await signMetaTxRequest(user, forwarder, tx);

    await forwarder.connect(relayer).execute(req.request, req.signature);

    expect(await p12Arcana.getVotingPower(BigNumber.from(user.address))).to.be.equal(200);
  });

  it('should owner update power successfully', async () => {
    await p12Arcana
      .connect(signer)
      ['updatePower(uint256,uint256,uint256)'](
        BigNumber.from(user.address),
        100,
